...
const Navbar = () => {
  ...
  const { showEventModal, setView,showEventDataModal,showMoreOpen }= contex 
  ...
  return (
    // sticky top-0
    <div className={`${(showEventModal || showMoreOpen || showEventDataModal)&& 'bg-red-100'} `}>
...
<header className={`text-gray-600 body-font  w-full mr-0  border`}>
  ...
  >
   ...
   {loggedAdmin && <div>
    ...
    <Link  to = '/main' className="ml-4 hover:text-gray-900 bg-purple-300 rounded-sm text-black px-4 py-1">Multiple Calendar </Link>
    <Link  to = '/offDays' className="ml-4 hover:text-gray-900 bg-purple-300 rounded-sm text-black px-4 py-1">Calendar off days setup </Link>
    </div>}
  </div>
</header>
