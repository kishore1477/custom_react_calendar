...
const OverlayDayView = () => {
   ...
   const {selectedDate, setSelectedDate, setView, savedEvents, setShowEventModal, showEventModal, selectedUsersEventFromLs} = contex
...
const {overlaydate} = routeParams
// console.log("date from useParamas:", date)
const category = overlaydate.split("$")
...
// console.log("date formtt:", date.format("DD-MM-YY"))
    ...
    function renderEvent(hour) {
        ...
        return selectedUsersEventFromLs.map(event => {
                            ...
                            {startDateTime === hour && (
                                <div className=" day-event ">
                                    <div 
                                        ...
                                        <h4 className='text-black'>{event.auditNo || "Untitled"}/{event.user}</h4>
                                    </div>
                                </div>
                            )}
                        </div>
